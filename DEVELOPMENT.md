...
**つまみは出しっぱなしにする。** Firefox の既定は重ね置きの細い棒で、触っている
間しか出てこない。送り箱だと気づけないので、`scrollbar-color` を `auto` 以外に
して場所を取る昔ながらの棒にする (Firefox はこの指定があると重ね置きをやめる)。
色は `--scroll-thumb` / `--scroll-track` で明暗どちらにも置いた。`--border` を
使い回したら薄すぎて、あるのに見えなかった。溝は `scrollbar-gutter: stable` で
先に空けておき、話数の数で文字の右端が動かないようにする。
