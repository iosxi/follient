...
作品ページ (`/manga/<作品>/`) に並ぶ話数のリンクを**全部**拾い、新しい順に
カードへ出す。1 サイト専用なので設定で切れる。既定は入り。

**数を絞っても取得は軽くならない。** ページの HTML は 1 回読むだけで、その中の
話数リンクは `chaptersFromDoc` がどのみち全部見て回っている。v20 で 3 つに
絞っていたのは「カードが縦に伸びる」という見た目の都合だけだった。増えるのは
保存する JSON の大きさで、1 話あたり 100 バイト程度しかない (500 話で 50KB)。

見た目のほうは `.chapters` を **3 行分の高さの送り箱**にして片を付けた。
`max-height: calc(var(--chapter-row) * 3 + var(--chapter-gap) * 2)` +
`overflow-y: auto`。何話あってもカードの丈は変わらないので masonry の貼り直しが
起きず、3 話に満たない作品は `max-height` なので縮む (空の行は出ない)。
縦並びの flex は丈を絞ると中身が潰れにいくので、`a` に `flex: none` が要る。

#### 棒は土台のものを使わない

**`scrollbar-width` / `scrollbar-color` では棒を出しっぱなしにできない。**
Windows 11 の Firefox の棒は重ね置きで、触っている間しか出てこない。この 2 つの
プロパティでできるのは「土台の棒に色を塗る」ことと「細くする・消す」ことだけで、
出し入れの挙動は OS 側の「スクロールバーを常に表示する」に従う。ページからは
決められない。

v23 に至るまで、この思い込みのまま 3 度も色をいじって直らなかった。

| やったこと | 変わったもの | 消える性質 |
| --- | --- | --- |
| `scrollbar-color: var(--border) ...` | 色 (薄すぎ) | 直らない |
| 透過を `0.34` に濃く | 色 | 直らない |
| 透過をやめてベタ色に | 色 | 直らない |

**そこで `.chapter-bar` / `.chapter-thumb` を自前で描く。** `.chapters` には
`scrollbar-width: none` を置いて土台の棒を消し、`.chapters-wrap` (箱と棒を包む
位置決めの親) の中に自分の棒を絶対配置する。棒を箱の外に置くのは、中に入れると
中身と一緒に送られてしまうため。カードに重ねたリンクより上に出すのはこの包みの
仕事 (`z-index: 2`)。でないと棒が押せない。

丈と位置は `syncChapterBar` が `scrollTop` / `scrollHeight` / `clientHeight` から
px で出す。割合を `%` で渡す手もあるが、500 話ある箱ではつまみが 1px を切って
摘めなくなる。最低の丈 (`CHAPTER_THUMB_MIN_PX`) を決めるなら px で計るしかない。
位置は `transform` で動かす (レイアウトが起きない)。`bindChapterBar` が
`setPointerCapture` で掴みを取り、溝を押したときはつまみの真ん中がそこへ来るよう
飛ばす。`pointerdown` で `preventDefault()` するのを忘れないこと。掴んだつもりで
下のリンクが開く。

**濃さはカードの面からの距離で決める。** 色を替えるときの目安。暗い配色で実際に
出た色 (カードの面は `--surface` = 32,32,39):

| 指定 | 実際に出る色 | 見え |
| --- | --- | --- |
| `rgba(255,255,255,0.09)` (`--border` の使い回し) | 52,52,58 | 見えない |
| `rgba(255,255,255,0.34)` | 108,108,113 | まだ薄い |
| `#7d7d90` (透過をやめただけ) | 125,125,144 | 前とほぼ同じ |
| `#b4b9cc` (いま) | 180,185,204 | 見える |

透過をやめても、同じくらいの濃さを置いたのでは何も変わらない。alpha があるか
どうかではなく、面から離れているかどうかが全てである。

**`overscroll-behavior` は指定しない。** 一度 `contain` にしたが、箱の一番上まで
戻したあとカーソルを箱に置いたままでは一覧そのものが動かせなくなった。既定
(`auto`) なら端まで来たところで背後へ渡る。Firefox は同じ 1 回の操作の途中では
すぐに渡さない (scroll latching) ので、送っている最中に一覧が飛ぶこともない。

**`<a>` は一度に全部置かない。** カードは画面に入ると二度と消えないので、
500 件のフォルダを一巡すると置いた `<a>` がそのまま積み上がる。1 作 300 話なら
15 万本になり、`:visited` はその 1 本ずつが履歴への問い合わせになる。見えている
のは 3 行だけなので `CHAPTER_CHUNK` (24) ずつ置き、箱を底から
`CHAPTER_REACH_PX` まで送ったところで継ぎ足す。まだ置いていないぶんは
`chapterRest` (WeakMap) が箱ごとに持つ。`scroll` の聞き役は箱 1 つにつき
1 回だけ付ける (`dataset.moreBound`)。中身は WeakMap から引くので、話数を
並べ直しても付け替えは要らない。
