...
### 撮影ウィンドウは「生成時から最小化」でしか隠せない

実測した結果は次のとおり (`screen.avail=3072x1680` の環境):

| やり方 | 結果 |
| --- | --- |
| 画面外へ配置 (left を負値 / 画面幅超) | Firefox が画面内へクランプする。隠せない |
| 生成後に `windows.update` で画面外へ | 同上。クランプされる |
| `create` で `width`/`height` と `state` を併用 | 例外。同時指定できない |
| `create({state:'minimized'})` (大きさ指定なし) | **成功**。`left=-25600` で一度も描画されない |
| 生成後に `update({state:'minimized'})` | 最小化はできるが、それまでの間は画面に見える |

そして **最小化したウィンドウでも `tabs.captureTab` は中身を返す**。
`example.com` で確認したところ、通常表示が 1232x827 / 色数 12、生成時から
最小化したものが 1732x1077 / 色数 14 と、どちらも実際の描画結果が得られた。

したがって `create({ state: 'minimized' })` が唯一の正解。大きさは指定できない
ので Firefox の既定に任せる (後処理で切り詰めて縮小するため支障は無い)。

150 秒・398 サンプルのウィンドウ監視で、画面内に余分なウィンドウが現れた回数は
0 回であることを確認済み。
