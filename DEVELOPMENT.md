...
隠しタブは利用者のウィンドウと同じ大きさになる。3〜4K のウィンドウだと
ビューポートが 3000 CSS px 近くになり (実測 `[innerW, innerH, dpr] = [2986, 1508, 1.25]`)、
そのまま 640px に縮めるとカード上では約 0.2 倍。何のサイトか判別できない。

対策は 2 つを組み合わせる。

1. `tabs.setZoom(tabId, 1.75)` で拡大する。ビューポートが 2986 -> 1679 CSS px に
   狭まり、極端に横長のレイアウトが普通の幅に収まる
2. `tabs.captureTab` の `rect` でビューポートの左上 1100x690 CSS px だけを撮る

こうするとウィンドウの大きさに関係なく同じ読みやすさになる。`rect` は
CSS ピクセル指定で、`scale` と併用できる (どちらも Firefox で使える)。
ビューポートより大きい範囲を要求すると破綻するので、`tabs.executeScript` で
`innerWidth`/`innerHeight` を測って収めている。測れない場合は範囲指定なしで撮る。

**利用者のサイト別拡大率を壊さないこと。** Firefox は `setZoomSettings({scope:'per-tab'})`
を受け付けない (`Unsupported zoom settings`)。つまりズームは**オリジン単位**でしか
設定できず、しかもその値は**永続保存される**。単に「既定へ戻す」と、利用者が
そのサイトに設定していた拡大率を消してしまう。

そこで撮影前に `getZoom` で現在値を控え、`finally` で必ずその値へ戻す。
既定のままだったのか明示設定だったのかは `getZoom` だけでは分からないので、
`getZoomSettings().defaultZoomFactor` と比較し、既定と一致していた場合だけ
`setZoom(tabId, 0)` を使って明示設定を作らないようにしている。

実測で確認済み:

| ケース | 撮影前 | 撮影中 | 撮影後に別タブで開いた値 |
| --- | --- | --- | --- |
| 利用者が 130% を設定済み | 1.3 | 1.75 | **1.3 (保持)** |
| 既定のまま | 1 | 1.75 | **1 (明示設定を作らない)** |

| 定数 | 既定 | 意味 |
| --- | --- | --- |
| `CAPTURE_ZOOM` | 1.75 | 撮影時の拡大率 |
| `CAPTURE_WIDTH` | 1100 | 切り出す幅 (CSS px) |
| `CAPTURE_HEIGHT` | 690 | 切り出す高さ (CSS px) |
