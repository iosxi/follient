...
### ホスト権限を `<all_urls>` にしない理由

`<all_urls>` は `file:` も含むため、Firefox の権限一覧に「ユーザーのコンピューター
上のローカルファイルへのアクセス」が出る。OGP 取得も撮影も `^https?://` で
弾いていて（`background.js` の `isFetchable`、`capture.js` の撮影前判定）
ローカルファイルは一切読まないので、`http://*/*` と `https://*/*` だけを
要求する。ここを広げると要らない権限を求めることになる。
